...
import { ProgressBarPGSteps, ProgressBarPGNonRFQSteps } from './utils';
...
type ProgressBarPGProps = {
  activeStep: number;
  error?: boolean;
};

const ProgressBarPG = ({ activeStep, error = false }: ProgressBarPGProps) => {
  const [progress, setProgress] = useState(0);

  const { asset }: any = useContext(InvestmentOverviewPGContext);

  // Currently not using the substeps
  const activeSubStep = 0;

  const exchangeType = 'NSE';

  const finalSteps = asset?.isRfq
    ? ProgressBarPGSteps
    : ProgressBarPGNonRFQSteps;
